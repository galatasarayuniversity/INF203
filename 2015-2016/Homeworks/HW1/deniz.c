...
int r_strcount(char *string, char c) {
	...
	if(r_strchr(string,c)==0){
		...
		if(*string==c) return 1+r_strcount(string+1,c);
		else return r_strcount(string+1,c);
	}
}
...
int roman_number_to_int(char *string) {
    /* TODO
	if(r_strlen(string)==1) return roman_digit_to_int(*string) ;
	else if(r_strlen(string)>=2){
		if(string>=string+1){
			return roman_digit_to_int(*string)+ roman_digit_to_int(*(string+1));
		}
		else if(string<string+1){
			return roman_digit_to_int(*(string+1))-roman_digit_to_int(*string);
		}
	}
}
