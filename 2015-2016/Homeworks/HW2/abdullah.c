...
void matmult_fast(int n, int Xpitch, const double X[], int Ypitch,
  ...
  matmult_fast(new_n, new_Xpitch, A, new_Ypitch, T, new_Zpitch, Z,
               min_mat_recurse);
  P[0] = Z;
  /* TODO: P1 = (A + B)*H */
  matadd(new_n, new_Ypitch, A, new_Ypitch, B, new_Ypitch, T);
  matmult_fast(new_n, new_Xpitch, T, new_Ypitch, H, new_Zpitch, Z,
               min_mat_recurse);
  P[1] = Z;
  /* TODO: P2 = (C + D)*E */
  matadd(new_n, new_Ypitch, C, new_Ypitch, D, new_Ypitch, T);
  matmult_fast(new_n, new_Xpitch, T, new_Ypitch, E, new_Zpitch, Z,
               min_mat_recurse);
  P[2] = Z;
  /* TODO: P3 = D*(G - E) */
  matsub(new_n, new_Ypitch, G, new_Ypitch, E, new_Ypitch, T);
  matmult_fast(new_n, new_Xpitch, D, new_Ypitch, T, new_Zpitch, Z,
               min_mat_recurse);
  P[3] = Z;
  /* TODO: P4 = (A + D)*(E + H) */
  matadd(new_n, new_Ypitch, A, new_Ypitch, D, new_Ypitch, T);
  matadd(new_n, new_Ypitch, E, new_Ypitch, H, new_Ypitch, U);
  matmult_fast(new_n, new_Xpitch, T, new_Ypitch, U, new_Zpitch, Z,
               min_mat_recurse);
  P[4] = Z;
  /* TODO: P5 = (B - D)*(G + H) */
