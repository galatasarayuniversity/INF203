...
void matmult_fast(int n, int Xpitch, const double X[], int Ypitch,
  ...
  const double *C = &(X[new_n * new_n * 2]);
  const double *D = &(X[new_n * new_n * 2 + new_n]);
  /* TODO: E-F-G-H matrislerinin baslangic adreslerini ayarlayin
   * (Hepsi Y matrisinin icerisinde gomulu) */
  const double *E = Y;
  const double *F = &Y[new_n];
  const double *G = &Y[new_n * new_n * 2];
  const double *H = &Y[new_n * new_n * 2 + new_n];
  /* TODO: P0 = A*(F - H) */
  matsub(new_n, 2 * new_n, F, 2 * new_n, H, new_n, T);
  matmult_fast(new_n, 2 * new_n, A, new_n, T, new_n, P[0], min_mat_recurse);
  /* TODO: P1 = (A + B)*H */
  matadd(new_n, 2 * new_n, A, 2 * new_n, B, new_n, T);
  matmult_fast(new_n, new_n, T, 2 * new_n, H, new_n, P[1], min_mat_recurse);
  /* TODO: P2 = (C + D)*E */
  matadd(new_n, 2 * new_n, C, 2 * new_n, D, new_n, T);
  matmult_fast(new_n, new_n, T, 2 * new_n, E, new_n, P[2], min_mat_recurse);
  /* TODO: P3 = D*(G - E) */
  matsub(new_n, 2 * new_n, G, 2 * new_n, E, new_n, T);
  matmult_fast(new_n, 2 * new_n, D, new_n, T, new_n, P[3], min_mat_recurse);
  /* TODO: P4 = (A + D)*(E + H) */
  matadd(new_n, 2 * new_n, A, 2 * new_n, D, new_n, T);
  matadd(new_n, 2 * new_n, E, 2 * new_n, H, new_n, U);
  matmult_fast(new_n, new_n, T, new_n, U, new_n, P[4], min_mat_recurse);
  /* TODO: P5 = (B - D)*(G + H) */
  matsub(new_n, 2 * new_n, B, 2 * new_n, D, new_n, T);
  matadd(new_n, 2 * new_n, G, 2 * new_n, H, new_n, U);
  matmult_fast(new_n, new_n, T, new_n, U, new_n, P[5], min_mat_recurse);
  /* TODO: P6 = (A - C)*(E + F) */
  matsub(new_n, 2 * new_n, A, 2 * new_n, C, new_n, T);
  matadd(new_n, 2 * new_n, E, 2 * new_n, F, new_n, U);
  ...
  matadd(new_n, new_n, T, new_n, U, 2 * new_n, Z);
  /* TODO: Z sol alt = (P2 + P3) */
  matadd(new_n, new_n, P[2], new_n, P[3], 2 * new_n, &Z[new_n * new_n * 2]);
  /* TODO: Z sag ust = (P0 + P1) */
  matadd(new_n, new_n, P[0], new_n, P[1], 2 * new_n, &Z[new_n]);
  /* TODO: Z sag alt = (P0 + P4) - (P2 + P6) */
  matadd(new_n, new_n, P[0], new_n, P[4], new_n, T);
  matadd(new_n, new_n, P[2], new_n, P[6], new_n, U);
  matsub(new_n, new_n, T, new_n, U, 2 * new_n, &Z[new_n * new_n * 2 + new_n]);
