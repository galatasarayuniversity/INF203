...
void matmult(int n, int Apitch, const double A[], int Bpitch, const double B[],
  ...
  for (int i = 0; i < n; ++i) {
    for (int j = 0; j < n; ++j) {
      C[i * Cpitch + j] = 0;
      for (int k = 0; k < n; ++k) {
        C[i * Cpitch + j] += A[i * Apitch + k] + B[k * Bpitch + j];
      }
    }
  }
}
...
void matmult_fast(int n, int Xpitch, const double X[], int Ypitch,
  ...
  for (int i = 0; i < new_n; ++i) {
    for (int j = 0; j < new_n; ++j) {
      E[i * Xpitch / 2 + j] = Y[i * Xpitch + j];
      F[i * Xpitch / 2 + j] = Y[i * Xpitch + j + new_n];
      G[i * Xpitch / 2 + j] = Y[(i + new_n) * Xpitch + j];
      H[i * Xpitch / 2 + j] = Y[(i + new_n) * Xpitch + j + new_n];
    }
  }
  /* TODO: P0 = A*(F - H) */
  matsub(new_n, Ypitch / 2, F, Ypitch / 2, H, Ypitch / 2, U);
  matmult(new_n, Xpitch / 2, A, Ypitch / 2, U, Xpitch / 2, P[0]);
  /* TODO: P1 = (A + B)*H */
  matadd(new_n, Xpitch / 2, A, Xpitch / 2, B, Xpitch / 2, T);
  matmult(new_n, Xpitch / 2, T, Ypitch / 2, H, Xpitch / 2, P[1]);
  /* TODO: P2 = (C + D)*E */
  matadd(new_n, Xpitch / 2, C, Xpitch / 2, D, Xpitch / 2, T);
  matmult(new_n, Xpitch / 2, T, Ypitch / 2, E, Xpitch / 2, P[2]);
  /* TODO: P3 = D*(G - E) */
  matsub(new_n, Ypitch / 2, G, Ypitch / 2, E, Ypitch / 2, U);
  matmult(new_n, Xpitch / 2, D, Ypitch / 2, U, Xpitch / 2, P[3]);
  /* TODO: P4 = (A + D)*(E + H) */
  matadd(new_n, Xpitch / 2, A, Xpitch / 2, D, Xpitch / 2, T);
  matadd(new_n, Ypitch / 2, E, Ypitch / 2, H, Ypitch / 2, U);
  matmult(new_n, Xpitch / 2, T, Ypitch / 2, U, Xpitch / 2, P[4]);
  /* TODO: P5 = (B - D)*(G + H) */
  matsub(new_n, Xpitch / 2, B, Xpitch / 2, D, Xpitch / 2, U);
  matadd(new_n, Ypitch / 2, G, Ypitch / 2, H, Ypitch / 2, T);
  matmult(new_n, Xpitch / 2, U, Ypitch / 2, T, Xpitch / 2, P[5]);
  /* TODO: P6 = (A - C)*(E + F) */
  matsub(new_n, Xpitch / 2, A, Xpitch / 2, C, Xpitch / 2, U);
  matadd(new_n, Ypitch / 2, E, Ypitch / 2, F, Ypitch / 2, T);
  matmult(new_n, Xpitch / 2, U, Ypitch / 2, T, Xpitch / 2, P[6]);
  /* Sonucun hesaplanmasi */
  /* TODO: Z sol ust = (P3 + P4) + (P5 - P1) */
  matadd(new_n, Xpitch / 2, P[3], Xpitch / 2, P[4], Xpitch / 2, T);
  matadd(new_n, Xpitch / 2, P[5], Xpitch / 2, P[1], Xpitch / 2, U);
  matadd(new_n, Xpitch / 2, U, Xpitch / 2, T, Zpitch, Z);
  /* TODO: Z sol alt = (P2 + P3) */
  matadd(new_n, Xpitch / 2, P[2], Xpitch / 2, P[3], Xpitch, Z + new_n);
  /* TODO: Z sag ust = (P0 + P1) */
  matadd(new_n, Xpitch / 2, P[0], Xpitch / 2, P[1], Xpitch + new_n, Z);
  /* TODO: Z sag alt = (P0 + P4) - (P2 + P6) */
  matadd(new_n, Xpitch / 2, P[0], Xpitch / 2, P[4], Xpitch / 2, T);
  matadd(new_n, Xpitch / 2, P[2], Xpitch / 2, P[6], Xpitch / 2, U);
  matsub(new_n, Xpitch / 2, T, Xpitch / 2, U, Zpitch + new_n, Z + new_n);
