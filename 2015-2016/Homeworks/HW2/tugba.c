...
void matmult(int n, int Apitch, const double A[], int Bpitch, const double B[],
	...
	for (int i = 0; i < n; ++i) {
		for (int j = 0; j < n; ++j) {
			for (int k = 0; k < n; ++k) {
				C[i * Cpitch + j] = 0;
				C[i * Cpitch + j] = C[i * Cpitch + j] + A[i * Apitch + k]
						+ B[i * Bpitch + k];
			}
		}
	}
...
void matmult_fast(int n, int Xpitch, const double X[], int Ypitch,
		const double Y[], int Zpitch, double Z[], int min_mat_recurse) {
	...
	{
		int i;
		double **P = malloc(7 * sizeof(sz));
		for (i = 0; i < 7; i++) {
			P[i] = malloc(7 * sizeof(int));
			if (P[i] != NULL) {
				/* yer ayirma islemi basarili*/
			} else {
				/*yer ayirma islemi basarisiz*/
			}
		}
	...
	{
		int i;
		double **T = malloc(3 * sizeof(sz));
		for (i = 0; i < 3; i++) {
			T[i] = malloc(3 * sizeof(int));
			U[i] = malloc(3 * sizeof(int));
			if (T[i] != NULL && U[i] != NULL) {
				/* yer ayirma islemi basarili*/
			} else {
				/*yer ayirma islemi basarisiz*/
			}
...
int main(int argc, char *argv[]) {
	...
	/* TODO: X ve Y matrislerini rasgele doldurun */
	{
		...
		for (int i = 0; i < n; ++i) {
			for (int j = 0; j < n; ++j) {
				Y[i * pitch + j] = r * (2 * drand48() - 1);
				X[i * pitch + j] = r * (2 * drand48() - 1);
			}
		}
	}
