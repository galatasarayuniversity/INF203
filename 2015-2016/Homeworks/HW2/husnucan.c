...
/* Bir matrisi rasgele sayilarla dolduran fonksiyon */ /* n: sutun sayısı,
                                                          pitch: satir sayisi */
...
void matmult_fast(int n, int Xpitch, const double X[], int Ypitch,
  ...
  double P2[new_n * new_n];
  double P3[new_n * new_n];
  double P4[new_n * new_n];
  double P5[new_n * new_n];
  double
      P6[new_n * new_n]; /*sizin dediğiniz gibi heap'den yer vermeye çalıştım
                            ama yapamadım. */
  double *T = malloc(sz);
  double *U = malloc(sz);
  double *A = &X[0];
  ...
  matmult_fast(new_n, Xpitch, A, Xpitch / 2, T, Xpitch / 2, P0,
               min_mat_recurse); /* TODO: P0 = A*(F - H) */
  matadd(new_n, Xpitch, A, Xpitch, B, Xpitch / 2, U);
  matmult_fast(new_n, Xpitch / 2, U, Xpitch, H, Xpitch / 2, P1,
               min_mat_recurse); /* TODO: P1 = (A + B)*H */
  matadd(new_n, Xpitch, C, Xpitch, D, Xpitch / 2, U);
  matmult_fast(new_n, Xpitch / 2, U, Xpitch, E, Xpitch / 2, P2,
               min_mat_recurse); /* TODO: P2 = (C + D)*E */
  matsub(new_n, Xpitch, G, Xpitch, E, Xpitch / 2, T);
  matmult_fast(new_n, Xpitch, D, Xpitch / 2, T, Xpitch / 2, P3,
               min_mat_recurse); /* TODO: P3 = D*(G - E) */
  matadd(new_n, Xpitch, A, Xpitch, D, Xpitch / 2, U);
  matadd(new_n, Xpitch, E, Xpitch, H, Xpitch / 2, T);
  matmult_fast(new_n, Xpitch / 2, U, Ypitch / 2, T, Xpitch / 2, P4,
               min_mat_recurse); /* TODO: P4 = (A + D)*(E + H) */
  matsub(new_n, Xpitch, B, Xpitch, D, Xpitch / 2, T);
  matadd(new_n, Xpitch, G, Xpitch, H, Xpitch / 2, U);
  matmult_fast(new_n, Xpitch / 2, T, Ypitch / 2, U, Xpitch / 2, P5,
               min_mat_recurse); /* TODO: P5 = (B - D)*(G + H) */
  matsub(new_n, Xpitch, A, Xpitch, C, Xpitch / 2, T);
  matadd(new_n, Xpitch, E, Xpitch, F, Xpitch / 2, U);
  matmult_fast(new_n, Xpitch / 2, T, Ypitch / 2, U, Xpitch / 2, P6,
               min_mat_recurse); /* TODO: P6 = (A - C)*(E + F) */
  matadd(new_n, Xpitch / 2, P3, Xpitch / 2, P4, Xpitch / 2, U);
  matsub(new_n, Xpitch / 2, P5, Xpitch / 2, P1, Xpitch / 2, T);
  matadd(new_n, Xpitch / 2, U, Xpitch / 2, T, Xpitch,
         &Z[0]); /* TODO: Z sol ust = (P3 + P4) + (P5 - P1) */
  matadd(new_n, Xpitch / 2, P2, Xpitch / 2, P3, Xpitch,
         &Z[new_n * Xpitch]); /* TODO: Z sol alt = (P2 + P3) */
  matadd(new_n, Xpitch / 2, P0, Xpitch / 2, P1, Xpitch,
         &Z[new_n]); /* TODO: Z sag ust = (P0 + P1) */
  matadd(new_n, Xpitch / 2, P0, Xpitch / 2, P4, Xpitch / 2, U);
  matadd(new_n, Xpitch / 2, P2, Xpitch / 2, P6, Xpitch / 2, T);
  matsub(
      new_n, Xpitch / 2, U, Xpitch / 2, T, Xpitch,
      &Z[new_n * Xpitch + new_n]); /* TODO: Z sag alt = (P0 + P4) - (P2 + P6) */
  free(U);
  free(T);
}
...
Soru: Bu bölümlendirerek çarpma algoritması klasik algoritmaya göre zamansal
karmaşalıkta iyileşme ya da kötüleşme sağlar mı ? Açıklayınız.
Divide and Conquer ile çarpma işlemini alt problemlere bölme, klasik algoritmaya
göre daha farkı yoktur. Çünkü Divide and Conquer ile
yaparsak eğer 8 çarpma ve 4 toplama işlemi yapılacaktır.
Ve master teorem ile ifadesi şöyle olacaktır.
A(n) = 8 x A(n/2) + f( n2 ) ve  nlogba  = nlog28  = n3  n3 > n2 o zaman A(n) =
0(n3)
Klasik algoritmada bir while 2 for işlemi var ve hepsi n defa işlem yapıyor yani
klasik algoritmada
A(n) = 0(n3)'dir. Sonuç olarak ikisi de aynı performanstadır.
2.2.2  Karmaşıklık :
Strassen yönteminin klasik yönteme göre neden daha iyi olduğunu ve zaman
karmaşıklığını büyük-O gösterimiyle anlatınız.
Strassen yöntemiyle Ana problemi 7 tane recursive yarı büyüklükte problemle
çözüyoruz. Yani 7 tane n/2 çarpma yapıorz. Ve
n2 zamanda combine yapılıyor.  A(n) = 7 x A(n/2) + f(n2)
0(n(log2 7)) < 0(n3) sonuç olarak Strassen yöntemi daha kısa sürede çalışıyor bu
da onu performans bakımından daha iyi yapıyor. */
