...
//!! p bir pointer, a bir dizi : p++ GEÇERLİ çünkü p bir nesne.a++ GEÇERSİZ çünkü a bir nesne değil. AMA p++ , p + i, p + 2 gibi ifadelere atama yapamazsın!! YANİ tanımlamadığın bir pointera (p++, p + 2 vs) atama işlemi gerçekleştiremezsin.  
...
void matmult(int n, int Apitch, const double A[], int Bpitch, const double B[], int Cpitch, double C[]) 
{
   int i, k, j;
   
   
   for (i = 0; i < n; i++)  //Çağırılan matrisin içindeki çöp değerler 0 değerine dönüştürülür.
        for (j = 0; j < n; j++) 
            C[i * Cpitch + j] = 0;
    ...
    for (i = 0; i < n; i++)
        for (j = 0; j < n; j++)
            for (k = 0; k < n; k++)   
                C[i * Cpitch + j] += A[i * Apitch + k] * B[k * Bpitch + j];   //Algoritme karmaşıklığı O(n3) ! 
     
}

/* Matris toplami: S = A + B */
void matadd(int n, int Apitch, const double A[], int Bpitch, const double B[], int Spitch, double S[])         
{
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
...
/* Matris farki: S = A - B */
void matsub(int n, int Apitch, const double A[], int Bpitch, const double B[], int Spitch, double S[]) 
{
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            S[i*Spitch + j] = A[i*Apitch + j] - B[i*Bpitch + j];
...
//        -                                            -
//
void matmult_fast(int n, int Xpitch, const double X[], int Ypitch, const double Y[], int Zpitch, double Z[], int min_mat_recurse) 
{
  /* TODO: min_mat_recurse'e kucuk esit bir matris geldiyse klasik algoritmayi kullan */
  ...
  matmult_fast(new_n, Xpitch , A,  Ypitch , U, Zpitch , P[0], min_mat_recurse); //fonksiyonun içinde kendisini çağırdım ama onunla return ettirmedim yine de özyinelemeli olmuş oluyo dimi??
  ...
  /* TODO: Z sol alt */matadd(new_n, Zpitch, P[2], Zpitch, P[3], Zpitch, (double *) (X + Xpitch/2 * Xpitch) ); 

  /* TODO: Z sag ust */matadd(new_n, Zpitch, P[0], Zpitch, P[1], Zpitch,(double *) (X + Xpitch/2)); 
  
  /* TODO: Z sag alt */matadd(new_n, Zpitch, P[0], Zpitch, P[4], Zpitch, U);
                       matadd(new_n, Zpitch, P[2], Zpitch, P[6], Zpitch, T); 
                       matsub(new_n, Zpitch, U, Zpitch, T, Zpitch, (double *) (X + Xpitch/2 * Xpitch + Xpitch/2 ));
