...
void matmult(int n,
	...
	for(int i=0; i<n; i++){
		...
		for(int k=0; k<n; k++){
			...
			sonuc+= A[i*Apitch+k] * B[k*Bpitch+j] ;
			C[i*Cpitch+j]= sonuc;
		}
...
void matmult_fast(int n,
 ...
 matsub(n, Y+n/2, F, Y+ Ypitch+ n/2, H,T) ;
 matmult_fast(new_n, X, A, T, Zpitch,Z);
 double P0= Z;

  /* TODO: P1 = (A + B)*H */
 matadd(n, Y+n/2, F, Y+ Ypitch+ n/2, H,T) ;
  matmult_fast(new_n, X, A, T, Zpitch, Z,min_mat_recurse);
  double P1= Z;
  /* TODO: P2 = (C + D)*E */
  matadd(n, X + Xpitch, C, X+ Xpitch+ n/2, D,T) ;
   matmult_fast(new_n, Y, E, T, Z + Zpitch , Z,min_mat_recurse);
   double P2= Z;
  /* TODO: P3 = D*(G - E) */
   matsub(n, Y+ Ypitch, G, Y, G,T) ;
    matmult_fast(new_n, X+ Xpitch+ n/2, D, T, Zpitch, Z,min_mat_recurse);
    double P3= Z;
  /* TODO: P4 = (A + D)*(E + H) */
    matadd(n, X , A, X+ Xpitch+ n/2, D,T) ;
    matadd(n, Y , E, Y+ Ypitch+ n/2, H,U) ;
    matmult_fast(new_n, U, T, Zpitch, Z,min_mat_recurse);
    double P4= Z;
  /* TODO: P5 = (B - D)*(G + H) */
    matsub(n, X+n/2 , B, X+ Xpitch+ n/2, D,T) ;
       matadd(n, Y+Ypitch , G, Y+ Ypitch+ n/2, H,U) ;
       matmult_fast(new_n, U, T, Zpitch, Z,min_mat_recurse);
       double P5= Z;
  /* TODO: P6 = (A - C)*(E + F) */
       matsub(n, X , A, X+ Xpitch, C,T) ;
          matadd(n, Y , E, Y+n/2, F,U) ;
          matmult_fast(new_n, U, T, Zpitch, Z,min_mat_recurse);
          double P6= Z;
