...
PGMInfo pgm_read(const char *filename)
{
	 ...
	 */
	FILE *fp = fopen(filename, "rb");	//windows sistemi uzerinde de sorunsuz calisabilmesi icin b eklendi
	if (fp == NULL) {
	...
	printf("%d\n", pgm_info.max_pixel_value);
//    sscanf(line, "%c", &pgm_info.max_pixel_value);
...
//              strlen ile okudğun şeyin boyutu alcaksn, son indextekini = NULL
		}
	...
	case '5':
		/* TODO: Binary PGM */
		read = fread(pgm_info.pixels, sizeof(char), pgm_info.width * pgm_info.height, fp);
		//    sscanf(line, "%d %d\n", &pgm_info.width, &pgm_info.height);
		break;
	}
	fclose(fp);
...
int pgm_write(const char *filename, PGMInfo pgm_info)
{
	...
	FILE *fp = fopen(filename, "wb");	//windows sistemi uzerinde de sorunsuz calisabilmesi icin b eklendi
	if (fp == NULL) {
		return 1;
	}
...
*/
	...
	case '5':
		...
		fwrite(pgm_info.pixels, sizeof(char), (pgm_info.height * pgm_info.width), fp);
		break;
