...
PGMInfo pgm_read(const char *filename)
{
	...
	/* TODO: Dosyayi acin. fopen ile okuma modunda aç Eger dosya acilamazsa pgm_info'nun error
	 * uyesini PGM_ERROR_READ yapip fonksiyonu return ettirin.
	 */

	/* bu error read gibi olaylar daha önceden tanımlanmış!
	 *
	 *
	 */
	...
	if (pgm == NULL) {
		...
		return pgm_info;	/* burada ne ile return ettirmem gerektiğini tam bulamadım */
	}
	 ...
	 */
	if (pgm_info.signature[0] != 'P' && pgm_info.signature[1] != '2' && pgm_info.signature[1] != '5') {
		fclose(pgm);
	...
	/* TODO: pgm_info.pixels icin malloc() ile yer ayiralim.- width*height-
	 * Bir piksel 1 bayt yer istiyor, unutmayalim.
	 */
	pgm_info.pixels = malloc(pgm_info.width * pgm_info.height * sizeof(char));	/* bunlar char mı emin degilim ama 1 bayt ihtiyacı var diye char sectim */

	/* TODO: malloc() ile yer ayrilamazsa dosyayi kapatin, error'u
	 * PGM_ERROR_MALLOC yapip fonksiyonu return --pgminfo-- ettirin.*/
	...
	case '2':
		/* TODO: Önce binatry'i bitir sonra ascii ekle __ ASCII PGM __ oku dönüştür atoi ile arraye yaz */
	...
	case '5':
		/* TODO: Binary PGM dosyadan w*h kadar bayt'ı oku */
...
int pgm_write(const char *filename, PGMInfo pgm_info)
{
	...
	/* TODO: Baslik yapisini fprintf() ile dosyaya yazin, p2,yorum,maxpixeldeğeri,.. */
	fprintf(pgm, "%s\n%s\n%d %d\n%d\n", pgm_info.signature, pgm_info.comment, pgm_info.width, pgm_info.height, pgm_info.max_pixel_value);
	/* TODO: 2 farkli dosya bicimi, 2 farkli yazma bicimi, p2 p5 ayrımını yap */
	...
	case '2':
		...
		for (j = 0; j < pgm_info.width * pgm_info.height; j++) {
			fprintf(pgm, "%d\n", pgm_info.pixels[j]);
		}
		break;
	case '5':
