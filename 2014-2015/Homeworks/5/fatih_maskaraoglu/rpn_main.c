...
/* todo yazan kısımları doldurdum ancak kodu calıstırınca her zaman -1 alıyorum.
 * bu da sanırım push fonksiyonumun doğru calısmadıgını gösteriyor, rakamların sayı karsılıklarını
 * gostermeyi unutmusum internetteki çözümleri denedim basaramadım yorum olarak eklerseniz o
 * değişimi sevinirim. kolay gelsin.
 */
...
double evaluate_RPN(char *expr, struct stackNode **top)
{
	...
	for (i = 0; i != '\0'; i++) {
		...
		double a = gecici->data;
		if (a > 57 || a < 48) {

			stack_push(*top, gecici->data);
			...
			double sonuc;
			sonuc = compute_operation(popVal1, popVal2, gecici->data);
			stack_push(top, sonuc);
