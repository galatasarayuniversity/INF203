...
double compute_operation(double op1, double op2, char operation)
{
	...
	case '/':
		return op1 / op2;
	default:
		printf("switch -case default value \n ");
	}
/*	if(operation == '+')
		return op1 + op2 ;
	else if(operation == '-')
...
double evaluate_RPN(char *expr, struct stackNode **top)
{
	...
	for (i = 0; expr[i] != '\0'; i++) {
		 ...
		 * Soru: Bir karakteriexitn sayi karsiligini nasil buluyorduk? */
		if (*expr != '*' && *expr != '/' && *expr != '+' && *expr != '-') {
			...
			donus_degeri = compute_operation(popVal1, popVal2, expr[i]);
			//To be more clear , i wrote these 2 function separately
			stack_push(top, donus_degeri);
